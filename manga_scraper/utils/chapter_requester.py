...
def request_next_chapter(spider, chapter_source):
    ...
    """
    chapters = list(chapter_source.values())
    
    # Check if current chapter is incomplete, retry downloading if necessary
    current_key = list(chapter_source.keys())[spider.chapter_index]
    if not spider.chapter_completed_map.get(current_key):
        print(f"\n🔄 Chapter {current_key}: Resuming download...")
        meta = {'chapter': current_key}
        if getattr(spider, 'use_playwright', False):
            meta = setup_playwright_meta(meta, spider.image_selector)
            
        yield scrapy.Request(
            url=spider.url_template.format(chapter=chapters[spider.chapter_index]),
    ...
    # Proceed to next chapter only if current chapter is complete
    while spider.chapter_index + 1 < len(chapters):
        ...
        if spider.chapter_completed_map.get(key):
            print(f"\n✅ Chapter {key}: Already downloaded. Skipping...")
